...
class PurchaseRequestSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
            ...
            # These fields are generated by the view or are just for display, 
            # but their presence in validated_data confuses DRF's internal assertion.
            # We pop them to ensure validated_data only contains PurchaseRequest model fields.
            validated_data.pop('manager_name', None)
            validated_data.pop('request_date', None)
            
            # 3. Create the parent object (PurchaseRequest)
            # validated_data is now clean and ready for model creation.
            purchase_request = PurchaseRequest.objects.create(**validated_data) 
            
            # 4. Create the nested items
            for item_data in items_data:
