...
# NEW VALIDATION LOGIC
    def validate(self, data):
        ...
        if data.get('medicine') is None:
            ...
            # CRITICAL: Since units/item is unknown, we set it to 'N/A' for the snapshot
            data['units_per_item'] = 'N/A'

            # Suggested amount is irrelevant for manual entries
            data['suggested_amount'] = 0 

            # If the contact number is missing, default it to an empty string to prevent errors
            if not data.get('supplier_contact_num_snapshot'):
                 data['supplier_contact_num_snapshot'] = ''
...
class PurchaseRequestSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        # 1. CRITICAL: POP the nested field FIRST.
        items_data = validated_data.pop('items')
        
        # 2. FIX: Explicitly POP the read_only fields
        validated_data.pop('manager_name', None)
        validated_data.pop('request_date', None)
        
        # 3. Create the parent object (PurchaseRequest)
        purchase_request = PurchaseRequest.objects.create(**validated_data) 
        
        # 4. Create the nested items - MODIFICATION IS HERE
        for item_data in items_data:
            
            # CRITICAL LOGIC: Determine the source based on the presence of a Medicine FK.
            # When the list is submitted from the main PR page, only existing Medicine IDs 
            # or completely new (manual) items are included.
            
            # If a Medicine ID is provided, the item comes from the forecast/low stock list 
            # and should be marked as FORECAST.
            if item_data.get('medicine'):
                item_data['source'] = 'FORECAST'
            else:
                # If no Medicine ID is provided, it is a new/unlisted item, mark it MANUAL.
                # This is for items added manually by the user on the main PR list.
