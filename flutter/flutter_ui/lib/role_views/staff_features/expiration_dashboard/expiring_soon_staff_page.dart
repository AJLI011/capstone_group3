...
class ExpiringSoonStaffPage extends StatefulWidget {
  const ExpiringSoonStaffPage({super.key});
...
class _ExpiringSoonStaffPageState extends State<ExpiringSoonStaffPage> {
  ...
  List<dynamic> _fullExpiringSoonStocks = [];
  // This is the list that will be displayed in chunks and filtered by search
  List<dynamic> _filteredExpiringSoonStocks = [];
  ...
  bool _hasMore = true;
  int _nextIndex = 0; // The index to start loading from in _fullExpiringSoonStocks
  ...
  void _filterExpiringSoonStocks() {
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        _filteredExpiringSoonStocks = _fullExpiringSoonStocks.sublist(0, _nextIndex);
      } else {
        // Otherwise, filter the entire fetched data based on the query
        _filteredExpiringSoonStocks = _fullExpiringSoonStocks.where((stock) {
  ...
  void _loadMoreStocks() {
    ...
    Future.delayed(const Duration(milliseconds: 500), () {
      ...
      final int end = newLength > _fullExpiringSoonStocks.length ? _fullExpiringSoonStocks.length : newLength;

      setState(() {
        _filteredExpiringSoonStocks.addAll(_fullExpiringSoonStocks.sublist(_nextIndex, end));
        _nextIndex = end;
        _isLoading = false;
        if (_nextIndex >= _fullExpiringSoonStocks.length) {
          _hasMore = false;
        }
      });
    });
  ...
  Future<void> _fetchAllExpiringSoonStocks() async {
    ...
    final String url = 'http://192.168.0.100:8000/api/medicines/expiring-soon/';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _fullExpiringSoonStocks = fetchedData;
          _isLoading = false;
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: _fullExpiringSoonStocks.isEmpty && _isLoading
              ? const Center(child: CircularProgressIndicator())
              : _filteredExpiringSoonStocks.isEmpty && _searchController.text.isNotEmpty
                ? const Center(child: Text('No matching medicines found.'))
                : _filteredExpiringSoonStocks.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemCount: _filteredExpiringSoonStocks.length + (_hasMore && _searchController.text.isEmpty ? 1 : 0),
                      itemBuilder: (context, index) {
                        if (index == _filteredExpiringSoonStocks.length) {
                          return const Center(
                            child: Padding(
                        ...
                        final stock = _filteredExpiringSoonStocks[index];
                        return Container(
