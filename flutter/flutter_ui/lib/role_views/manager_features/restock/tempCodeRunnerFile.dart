...
class RestockDetailsPage extends StatefulWidget {
  // 🎯 MODIFIED: Update the constructor for the PR linking flow
  final Map<String, dynamic>? medicine; // Old flow: pass full map
  final int? medicineId; // New flow: pass ID
  final int? initialRestockAmount; // New flow: pass required quantity

  const RestockDetailsPage({
    super.key,
    this.medicine, // Now optional
    this.medicineId, // New
    this.initialRestockAmount, // New
  }) : assert(medicine != null || medicineId != null, 
    'Either the full medicine map or the medicine ID must be provided.');
...
class _RestockDetailsPageState extends State<RestockDetailsPage> {
  ...
  // 🎯 NEW: State to hold the final medicine data
  Map<String, dynamic>? _currentMedicine; 
  bool _isLoading = true; // Start loading until data is ready
  late bool _isSyrup = false; // Initialized in _fetchOrUseMedicineData

  @override
  void initState() {
    super.initState();
    _fetchOrUseMedicineData(); // Kick off data loading/setup

    // Listeners are added inside _fetchOrUseMedicineData after data is ready
  }

  // 🎯 NEW: Logic to determine how to get medicine data
  Future<void> _fetchOrUseMedicineData() async {
    Map<String, dynamic>? data;

    // 1. If full map is provided (old flow)
    if (widget.medicine != null) {
      data = widget.medicine;
    } 
    // 2. If ID is provided (new PR linking flow)
    else if (widget.medicineId != null) {
      data = await _fetchMedicineDetails(widget.medicineId!);
    }
    
    // Set the state variables once data is acquired
    if (data != null) {
      _currentMedicine = data;
      _isSyrup = _currentMedicine!['dosage_form']?.toLowerCase() == 'syrup';

      // Use the restock amount from the PR if provided, otherwise use the
      // default restock_quantity from the medicine details, or default to 0.
      final amount = widget.initialRestockAmount ?? _currentMedicine!['restock_quantity'];
      _quantityController.text = amount?.toString() ?? '0';

      // Add listeners after controllers are initialized
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Failed to load medicine details for restock.')),
          );
        }
        return null;
      }
  ...
  // --- LOGIC (UPDATED TO USE _currentMedicine) ---
  ...
  Future<void> _submitRestock() async {
    if (!_formKey.currentState!.validate() || _currentMedicine == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please fill out all required fields or data is loading.')),
      );
      return;
    }
    ...
    // 🎯 UPDATED: Use _currentMedicine for medicine ID
    final restockData = {
      'medicine': _currentMedicine!['id'], 
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        if (mounted) {
          ...
          // Pop twice if coming from AddMedicineScreen to return to NewMedicinePrScreen
          if (widget.medicineId != null) {
             // Pop RestockDetailsPage (true)
             Navigator.pop(context, true); 
             // The calling AddMedicineScreen will have already been replaced, 
             // so a single pop returns to the screen before AddMedicineScreen (NewMedicinePrScreen)
          } else {
             Navigator.pop(context, true);
          }
        }
  ...
  // --- ENHANCED UI WIDGETS (UPDATED TO USE _currentMedicine) ---

  Widget _buildMedicineInfoCard() {
    if (_currentMedicine == null) return const SizedBox.shrink();
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // 🎯 UPDATED: Use _currentMedicine
            _infoRowEnhanced(Icons.medication_outlined, 'Name', formatValue(_currentMedicine!['name'])),
            _infoRowEnhanced(Icons.science_outlined, 'Generic Name', formatValue(_currentMedicine!['generic_name'])),
            _infoRowEnhanced(Icons.category_outlined, 'Category', formatValue(_currentMedicine!['category'])),
            _infoRowEnhanced(Icons.line_weight, 'Dosage Form', formatValue(_currentMedicine!['dosage_form'])),
            _infoRowEnhanced(Icons.money, 'Price', '₱${formatValue(_currentMedicine!['price'])}'),
          ],
        ),
      ),
    );
  ...
  // --- BUILD METHOD (UPDATED FOR LOADING STATE) ---
  ...
  Widget build(BuildContext context) {
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 10),
              Text('Loading medicine details...', style: TextStyle(color: _primaryColor)),
            ],
          ),
        ),
      );
    ...
    // Safety check: if loading finished but data is still null
    if (_currentMedicine == null) {
      return Scaffold(
        ...
        body: const Center(child: Text('Failed to load medicine data.', style: TextStyle(color: Colors.red))),
      );
    }
    
    final String? imageUrlFromWidget = _currentMedicine!['image'];
    String? fullImageUrl;
