...
class _PromoMedicinePageState extends State<PromoMedicinePage> {
  ...
  Future<void> fetchExpiringSoonMedicines() async {
    const String url = 'https://aaron.pythonanywhere.com/api/medicines/expiring-soon/';
    try {
  ...
  Future<void> setPromo(int inventoryId, String startDate, String endDate) async {
    ...
    final url = Uri.parse('https://aaron.pythonanywhere.com/api/inventory/$inventoryId/set-promo/');
  ...
  Future<void> removePromo(int inventoryId) async {
    ...
    final url = Uri.parse('https://aaron.pythonanywhere.com/api/inventory/remove-promo/');
    try {
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/json'},
