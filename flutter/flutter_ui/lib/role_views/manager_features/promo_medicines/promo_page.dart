...
class _PromoMedicinePageState extends State<PromoMedicinePage> {
  ...
  Future<void> fetchExpiringSoonMedicines() async {
    const String url = 'http://juliaally.pythonanywhere.com/api/medicines/expiring-soon/';
    try {
  ...
  Future<void> setPromo(int inventoryId, String startDate, String endDate) async {
    ...
    final url = Uri.parse('http://juliaally.pythonanywhere.com/api/inventory/$inventoryId/set-promo/');
  ...
  Future<void> removePromo(int inventoryId) async {
    ...
    final url = Uri.parse('http://juliaally.pythonanywhere.com/api/inventory/remove-promo/');
    try {
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/json'},
