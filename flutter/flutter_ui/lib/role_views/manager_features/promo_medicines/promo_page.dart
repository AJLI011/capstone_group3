...
class _PromoMedicinePageState extends State<PromoMedicinePage> {
  ...
  Future<void> _fetchAllPromoMedicines() async {
    ...
    const String url = 'http://192.168.0.100:8000/api/medicines/expiring-soon/';
    try {
  ...
  Future<void> setPromo(int inventoryId, String startDate, String endDate) async {
    ...
    final url = Uri.parse('http://192.168.0.100:8000/api/inventory/$inventoryId/set-promo/');
  ...
  Future<void> removePromo(int inventoryId) async {
    ...
    final url = Uri.parse('http://192.168.0.100:8000/api/inventory/remove-promo/');
    try {
      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/json'},
