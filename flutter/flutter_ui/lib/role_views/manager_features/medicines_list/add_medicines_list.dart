...
// import '../../manager_features/restock/restock_details.dart'; // REMOVED: No longer navigating to RestockDetailsPage
...
class AddMedicineScreen extends StatefulWidget {
  ...
  final int? restockAmount; // <--- KEPT: Needed for setting initial quantity and sending to backend
...
class _AddMedicineScreenState extends State<AddMedicineScreen> {
  ...
  // --- API URLS ---
  final String _medicineCreateUrl = 'http://192.168.1.20:8000/api/medicine/create/';
  final String _linkPrItemBaseUrl = 'http://192.168.1.20:8000/api/restock/purchase-request/link-new-medicine/';
  ...
  Future<bool> _checkBarcodeExistence(String barcode) async {
    ...
    final url = Uri.parse('http://192.168.1.20:8000/api/medicines/check_barcode/$barcode/');
    try {
  ...
  Future<void> _fetchSuppliers() async {
    ...
    // IMPORTANT: Replace with your computer's actual local IP address!
    final url = Uri.parse('http://192.168.1.20:8000/api/suppliers/');
    try {
      final response = await http.get(url);
      if (response.statusCode == 200) {
        setState(() {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        if (rawMedicineId is String) {
          // Try parsing the string ID to an integer
        ...
        // REMOVED: prRestockAmount logic is no longer needed for navigation/redirection.
        /*
        ...
        */
        ...
        if (_isLinkingMode && widget.prItemId != null && newMedicineId > 0) {
          ...
          if (mounted) {
            // MODIFIED: Pop back to the previous screen (PR List/Detail) instead of pushing to RestockDetailsPage.
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Medicine registered and linked successfully!'), backgroundColor: _accentColor),
            );
            Navigator.of(context).pop(true); // Signal success back to the PR screen
          }
            ...
            child: Column(
              ...
              children: [
                      ...
                      children: [
                        Expanded(
                          child: TextFormField(
                            ...
                            //readOnly: _isLinkingMode, // The restock amount is pre-filled, but can still be edited before registration.
                            decoration: _inputDecoration('Restock Qty', Icons.low_priority).copyWith(
                              fillColor: _inputFillColor, // Keep editable, no grey background
                            ),
                            keyboardType: TextInputType.number,
                            validator: (value) {
                ...
                ElevatedButton.icon(
                    ...
                    final confirmed = await showDialog<bool>(
                      ...
                      builder: (context) {
                        return AlertDialog(
                          ...
                          content: Text(_isLinkingMode ? 'Are you sure you want to register this medicine and link it to the Purchase Request?' : 'Are you sure you want to add this medicine?'),
                          actions: [
                            TextButton(
                              onPressed: () => Navigator.of(context).pop(false),
                            ...
                            ElevatedButton(
                              ...
                              child: Text(_isLinkingMode ? 'Register & Link' : 'Add Medicine'), // Modified button text
                            ),
                          ],
                        );
                      },
                  ...
                  label: Text(
                    _isLinkingMode ? 'REGISTER MEDICINE' : 'ADD MEDICINE', // Modified button text
